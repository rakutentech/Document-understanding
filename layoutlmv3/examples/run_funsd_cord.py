...
import torch
from layoutlmft.models.layoutlmv3.Utils import *
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        print("load trainer args from {}".format(os.path.abspath(sys.argv[1])))
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        tokenizer_file=None,  # avoid loading from a cached filerai of the pre-trained model in another machine
        cache_dir=model_args.cache_dir,
        use_fast=True,
    ...
    def tokenize_and_align_labels(examples, augmentation=False):
        ...
        tokenized_inputs = tokenizer(
            ...
            max_length=config.word_embedding_size,
        )
        ...
        for batch_index in range(len(tokenized_inputs["input_ids"])):
            ...
            if data_args.visual_embed:
                ...
                if config.DA is not None:
                    # Use proposed data aug here
                    if config.DA_bbox:
                        img, bbox = resize_bbox(img, bbox, level=config.DA_level)
                    else:
                        img, _ = resize_bbox(img, bbox, level=config.DA_level)
                for_patches, _ = common_transform(img,
                                                  augmentation=augmentation)  # RandomResizedCropAndInterpolationWithTwoPic
                patch = patch_transform(for_patches)  # toTensor and Norm
                images.append(patch)
    ...
    # Tokenize all texts and align the labels with them.
    def tokenize_and_align_labels_4_eval(examples, augmentation=False):
        tokenized_inputs = tokenizer(
            examples[text_column_name],
            padding=False,
            truncation=True,
        ...
        for batch_index in range(len(tokenized_inputs["input_ids"])):
            ...
            if data_args.visual_embed:
                ...
                if config.DA_eval is not None:
                    # Use proposed data aug here
                    img = resize_bbox(img, bbox, level=config.DA_level)
    ...
    if training_args.do_eval:
        ...
        eval_dataset = eval_dataset.map(
            tokenize_and_align_labels_4_eval,
            batched=True,
            remove_columns=remove_columns,
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file=not data_args.overwrite_cache,
    ...
    def compute_metrics(p):
        ...
        true_predictions = [
            ...
            for prediction, label in zip(predictions[:config.word_embedding_size], labels[:config.word_embedding_size])
        ]
        true_labels = [
