...
#         self.y1_position_embeddings = nn.Embedding(config.max_2d_position_embeddings, config.coordinate_size)
        ...
        if self.config.cell_emb:
            self.row_position_embeddings = nn.Embedding(config.cell_embedding_dim, config.shape_size * 3)
            self.col_position_embeddings = nn.Embedding(config.cell_embedding_dim, config.shape_size * 3)
        else:
            self.row_position_embeddings = nn.Embedding(config.cell_embedding_dim, config.coordinate_size)
            self.col_position_embeddings = nn.Embedding(config.cell_embedding_dim, config.coordinate_size)
    ...
    def _calc_spatial_position_embeddings(self, bbox):
        ...
        if self.PE_type ==7 and self.config.pretrain_flag:
            left_position_embeddings = self.x_position_embeddings(bbox[:, :, 0])
            upper_position_embeddings = self.y_position_embeddings(bbox[:, :, 1])
            h_position_embeddings = self.h_position_embeddings(torch.clip(bbox[:, :, 3] - bbox[:, :, 1], 0, 1023))
            w_position_embeddings = self.w_position_embeddings(torch.clip(bbox[:, :, 2] - bbox[:, :, 0], 0, 1023))
            row_cell_embeddings = self.row_position_embeddings(bbox[:, :, 4])
            column_cell_embeddings = self.col_position_embeddings(bbox[:, :, 5])
        else:
            try:
                ...
                right_position_embeddings = self.x_position_embeddings(bbox[:, :, 2])
                lower_position_embeddings = self.y_position_embeddings(bbox[:, :, 3])
                if self.PE_type in [2,3,5]:
                    row_cell_embeddings = self.h_position_embeddings(bbox[:, :, 4])
                    column_cell_embeddings = self.w_position_embeddings(bbox[:, :, 5])
                elif self.PE_type in [6,7]:
                    row_cell_embeddings = self.row_position_embeddings(bbox[:, :, 4])
                    column_cell_embeddings = self.col_position_embeddings(bbox[:, :, 5])
            except IndexError as e:
                raise IndexError("The :obj:`bbox` coordinate values should be within 0-1000 range.") from e
            
            if self.config.PE_type !=5:
                h_position_embeddings = self.h_position_embeddings(torch.clip(bbox[:, :, 3] - bbox[:, :, 1], 0, 1023))
                w_position_embeddings = self.w_position_embeddings(torch.clip(bbox[:, :, 2] - bbox[:, :, 0], 0, 1023))
        ...
        if self.PE_type ==1:
        ...
        elif  self.PE_type==2 or self.PE_type==6:
            spatial_position_embeddings = torch.cat(
                [
                    left_position_embeddings,
                    upper_position_embeddings,
            ...
            raise RuntimeError("No such PE_type")
        return spatial_position_embeddings

    def _calc_cell_embedding(self, cell):
        row_cell_embeddings = self.row_position_embeddings(cell[:, :, 0])
        column_cell_embeddings = self.col_position_embeddings(cell[:, :, 1])
        cell_position_embeddings = torch.cat(
                [
                    row_cell_embeddings,
                    column_cell_embeddings
                ],
        ...
        return cell_position_embeddings
    ...
    def forward(
        ...
        if self.config.in_attn :
            ...
            if cell is not None:
                cell_position_embeddings = self._calc_cell_embedding(cell)
                embeddings  = embeddings + cell_position_embeddings
...
class LayoutLMv3SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.attention_type == "simple":
            mixed_query_layer = hidden_states
        elif self.config.attention_type == "default":
...
class LayoutLMv3Encoder(nn.Module):
    ...
    def _cal_2d_pos_emb(self, hidden_states, bbox):
        position_coord_x = bbox[:, :, 0]  # x1
        position_coord_y = bbox[:, :, 3]  # y3
...
def create_cell_layout_4_layoutlm(bboxes, max_x, max_y,scale=0.01, default_cell=[512,512]):
    ...
    for bbox in bboxes:
        key_point = bbox[:2]
        if key_point[1] == 1000:
            row_cell = row_coor_2_cell_dict[999]
        else:
            row_cell = row_coor_2_cell_dict[int(key_point[1])]
        if key_point[0] == 1000:
            col_cell = col_coor_2_cell_dict[999]
        else:
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        if not self.image_only:
            ...
            if self.config.sort_flag or self.config.PE_type in [2,5,6,7] or self.config.cell_emb or self.config.norm_coor:
                cell_inputs = []
                cell_coor_inputs = []
    ...
    #                     cells.append([1,1])
                ...
                cells = torch.from_numpy(np.array(cell_inputs)).cuda().int()
...
#                     idx = np.array( data[data!='10001000']).argsort(kind="mergesort")+1
                    idx = np.array( data[data!='10001000']).argsort(kind="mergesort")
                    length = len(idx)
                    new_idxs.append(idx)
                    #input_ids[i,1:length+1] = input_ids[i,idx]
                    input_ids[i, 0:length] = input_ids[i,idx]
                    bbox[i, : length,:] = bbox[i,idx,:]
                    token_type_ids [i, :length] = token_type_ids[i,idx]
                    #bbox[i,1:length+1,:] = bbox[i,idx,:]
                    #token_type_ids[i,1:length+1] = token_type_ids[i,idx]
                
                new_idxs = np.array(new_idxs)

#             bbox = torch.cat([cell_coors[:, :, :2], bbox[:, :, 2:]], dim=2)
            
            if self.config.norm_coor:
                bbox = torch.cat([cell_coors[:, :, :2], bbox[:, :, 2:]], dim=2)
            if self.config.PE_type in [1,4]:
                if self.config.cell_emb:
                ...
                if self.config.PE_type == 3:
                    cells = torch.ones(bbox.shape[0], bbox.shape[1], 2).cuda().int()
                embedding_output = self.embeddings(
                    input_ids=input_ids,
                    bbox=torch.cat([bbox[:,:,:4], cells], dim=2),
                    position_ids=position_ids,
                    token_type_ids=token_type_ids,
                    inputs_embeds=inputs_embeds,
                    past_key_values_length=past_key_values_length,
        ...
        if self.config.sort_flag:
            for i in range(sequence_output.shape[0]):
                idxs = new_idxs[i]
                length = len(idxs)
                tmp_idxs = [-1 for i in idxs]
                for idx, value in enumerate(idxs):
#                     tmp_idxs[value-1] = idx + 1
                    tmp_idxs[value-1] = idx
                
#                 sequence_output[i][1:1+length] = sequence_output[i][tmp_idxs]
                sequence_output[i][:length] = sequence_output[i][tmp_idxs]
...
class LayoutLMv3ForTokenClassification(LayoutLMv3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.layoutlmv3 = LayoutLMv3Model(config)  # main model
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        outputs = self.layoutlmv3(  # feed into main model
            input_ids,
            bbox=bbox,
